...
//创建数据库之类的
$con=mysql_connect('localhost', 'root', 'suyulong');
...
switch($act)
{
	case 'add':
		...
		break;
	case 'zan':
		$zanid=urldecode($_GET['zanid']);
		$num=urldecode($_GET['zannum']);
		
		$sql="UPDATE weibo  SET acc=$num+1 where id=$zanid";
		...
		break;
	case 'del':
		$delid=urldecode($_GET['delid']);
//		$num=urldecode($_GET['zannum']);
		
		$sql="DELETE FROM weibo where id=$delid";
